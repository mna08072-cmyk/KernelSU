...
#else
// Pre-5.10 kernels predate fsnotify_ops.handle_inode_event; serve the same
// watch through the legacy handle_event entry. free_mark is mandatory there
// (called unconditionally on mark teardown), so free our kzalloc'd mark.
